...
- `skills/repo-health--helpers/`: New subskill containing grading rubric + full script
  reference
- `skills/repo-health--phase-{0..10}/`: 11 new subskills, one per audit phase, callable
  only by the orchestrator

### Fixed

- Git branch `calm-beaver` now tracks `origin/main` (was previously tracking
  `origin/calm-beaver` which no longer exists on the remote)

### Removed

- `skills/repo-health/SKILL.md` (853-line monolith): Replaced by orchestrator (64 lines)
  with 12 subskills (720 lines total)
  - Monolith was too large, had no partial-invocation path, and coupled 10 independent
    audit dimensions into one file
